...
function setup() {
    ...
    spaceship = createSprite(displayWidth - 700, displayHeight - 200, 40, 40);
...
function missilerelease() {
    if (keyWentDown("SPACE")) {
        missile = createSprite(spaceship.x, displayHeight - 200, 20, 25);
...
function missilecollide(){
    if (astgrp.isTouching(missilegrp)) {
        for (var i = 0; i < astgrp.length; i++) {

            if (astgrp[i].isTouching(missilegrp)) {
                astgrp[i].destroy()
